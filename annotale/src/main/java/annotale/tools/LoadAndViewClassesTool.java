    ...
    public ToolParameterSet getToolParameters() {
        ...
        try {
            SelectionParameter sp = new SelectionParameter(DataType.PARAMETERSET,
                  new String[]{"Download current definition", "Load from local file"},
                  new Object[]{new SimpleParameterSet(), new SimpleParameterSet(builderFile)},
                  "Class definition source",
                  "Download current class definition (requires internet connection) or load definition from local file",
                  true);
    ...
    public ToolResult run(ToolParameterSet parameters, Protocol protocol, ProgressUpdater progress,
        ...
        if (sp.getSelected() == 0) {
            ...
            InputStream is = new URL(
                  "http://www.jstacs.de/downloads/class_definitions_current2.xml.gz").openStream();
            ...
            ress.add(new TextResult("Class builder download", "TALE class builder definition",
                  new FileRepresentation("", text), "xml", "Downloaded",
        ...
        reports.add(new TextResult("All TALE protein sequences",
              "Sequences of all TALEs in the class builder",
              new FileRepresentation("", allTales.toString()), "fasta", "LoadAndViewClasses",
              "fasta/as", true));
        ...
        reports.add(new TextResult("All TALE DNA sequences",
              "Sequences of all TALEs in the class builder",
              new FileRepresentation("", allTales.toString()), "fasta", "LoadAndViewClasses",
              "fasta/dna", true));
...
//System.out.println("by strain");
        ...
        while (it.hasNext()) {
            ...
            byStrainList.add(new TextResult("TALE DNA sequences (" + strainn + ")",
                  "Sequences of all TALEs for strain " + strainn,
                  new FileRepresentation("", talesByStrain.get(strain).toString()), "fasta",
                  "LoadAndViewClasses", "fasta/dna", true));
        }

        reports.add(
              new ResultSetResult("TALEs by strain", "The TALE sequences of all strains", null,
                    new ResultSet(byStrainList)));
...
//System.out.println("List 1");	
        ...
        for (int i = 0; i < tales.length; i++) {
            ...
            if (strain != null) {
                ...
                if (!strainMap.containsKey(key)) {
                    strainRes.add(
                          new ResultSet(new Result[]{new CategoricalResult("Strain", "", strain),
                                new CategoricalResult("Accession", "", acc)}));
        ...
        if (woStrain > 0) {
            strainRes.add(new ResultSet(
                  new Result[]{new CategoricalResult("Strain", "", "No strain annotation"),
                        new CategoricalResult("Accession", "", woStrain + " TALEs")}));
        }
        ListResult lr = new ListResult("List of TALEs",
              "The list of all TALEs in the class builder", null,
              taleList.toArray(new ResultSet[0]));
        reports.add(lr);

        ListResult lr2 = new ListResult("List of strains",
              "The list of all strains in the class builder", null,
              strainRes.toArray(new ResultSet[0]));
        reports.add(lr2);
        ...
        reports.add(new TextResult("List of classes",
              "The list of all TALEs classes and corresponding distance trees",
              new FileRepresentation("", familyReport.toString()), "txt", "LoadAndViewClasses",
              "txt", true));

        ResultSet repSet = new ResultSet(reports.toArray(new Result[0]));

        ress.add(new ResultSetResult("Lists of classes, strains and TALEs",
              "Details on the classes, strains and TALEs in the current class builder", null,
              repSet));

        ress.add(new PlotGeneratorResult("Tree of classes", "The tree of class similarities",
              new FamilyGroupPlotter.FamilyGroupPlotGenerator(famTree), true));
        ...
        for (int i = 0; i < fams.length; i++) {
            ...
            PlotGeneratorResult pgr = new PlotGeneratorResult("Class " + fams[i].getFamilyId(),
                  "Plot of the tree of the TALEs in this class", fams[i], true);
            classTrees.add(pgr);
            progress.setCurrent(1.0 + i / (double) fams.length);
        }

        ress.add(new ResultSetResult("Class trees", "Trees of all TALE classes", null,
              new ResultSet(classTrees)));

        ResultSet set = new ResultSet(ress.toArray(new Result[0]));

        return new ToolResult("Result of " + getToolName(), "", null, set, parameters,
    ...
    public String getHelpText() {
        try {
            return FileManager.readInputStream(LoadAndViewClassesTool.class.getClassLoader()
                  .getResourceAsStream("annotale/tools/LoadAndViewClassesTool.txt")).toString();
        } catch (IOException e) {
            e.printStackTrace();
            return "";
        }
