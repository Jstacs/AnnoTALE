...
public class SplitTALEs {
    ...
    public static TALE[] split(String id, Sequence tale, Protocol protocol) throws Exception {
        ...
        Sequence foundStart = prot.getSubSequence(0,
              al2.substring(0, alignmentIndex).replaceAll("-", "").length());
        ...
        if (tale.getAlphabetContainer().checkConsistency(DNAAlphabetContainer.SINGLETON)) {
            ...
            for (int j = 0; j < dnaRepeat.length; j++) {
                dnaRepeat[j] = new Repeat(
                      tale.getSubSequence(off * 3, foundRepeats[j].getRepeat().getLength() * 3));
        ...
        if (dnaTALE != null) {
            try {
                ...
                protocol.appendWarning(
                      "Translated version of TALE " + id + " could not be created. Reason\n"
                            + e.getMessage() + "\n");
            }
        } else {
            try {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < ds.getNumberOfElements(); i++) {
            ...
            while (true) {
                ...
                Sequence template = Sequence.create(protAlph,
                      first.toString() + repeat.toString() + last);

                PairwiseStringAlignment psa = align.getAlignment(AlignmentType.GLOBAL, template,
                      prot);
                if (psa.getCost() > best) {
                    break;
                } else {
            ...
            for (int j = 0; j < dnaRepeat.length; j++) {
                dnaRepeat[j] = dna.toString()
                      .substring(off * 3, (off + foundRepeats[j].length()) * 3);
                off += foundRepeats[j].length();
            }
            String dnaLastRepeat = dna.toString()
                  .substring(off * 3, (off + foundLastRepeat.length()) * 3);
            off += foundLastRepeat.length();
            String dnaEnd = dna.toString().substring(off * 3, (off + foundEnd.length()) * 3);

            String id = (String) dna.getSequenceAnnotationByType("unparsed comment line", 0)
                  .getResultAt(0).getValue();
