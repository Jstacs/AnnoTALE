...
public class RenameTool implements JstacsTool {
    ...
    public ToolParameterSet getToolParameters() {
        FileParameter names = new FileParameter("Rename Table",
              "A tab-separated table containing the old name in the first column and the new name in the second column. Output \"TALE names\" of \"TALE Class Assignment\" tool.",
              "tsv", true);

        FileParameter input = new FileParameter("Input file",
              "The input Genbank or GFF3 file that should be renamed.", "gb,gbk,genbank,gff,gff3",
              true);
    ...
    public ToolResult run(ToolParameterSet parameters, Protocol protocol, ProgressUpdater progress,
        ...
        if (input.getFileContents().getExtension().equalsIgnoreCase("gff")
              || input.getFileContents().getExtension().equalsIgnoreCase("gff3")) {
            ...
            TextResult fres = new TextResult("Renamed GFF: TALE loci",
                  "Renamed TALEs in GFF format", new FileRepresentation("", res.toString()), "gff3",
                  "Rename", null, true);

            protocol.append("Renamed TALEs in GFF file.\n");

            return new ToolResult("Result of " + getToolName() + " (GFF)",
                  getToolName() + " on \"" + input.getFileContents().getFilename() + "\"", null,
                  new ResultSet(fres), parameters, getToolName(),
            ...
            BufferedReader br = new BufferedReader(
                  new StringReader(input.getFileContents().getContent()));
            ...
            TextResult fres2 = new TextResult("Renamed Genbank: TALEs",
                  "Renamed TALEs in Genbank format", new FileRepresentation("", cont), "gb",
                  "Rename", null, true);

            protocol.append("Renamed TALEs in Genbank file.\n");

            return new ToolResult("Result of " + getToolName() + " (Genbank)",
                  getToolName() + " on \"" + input.getFileContents().getFilename() + "\"", null,
                  new ResultSet(fres2), parameters, getToolName(),
                  new Date(System.currentTimeMillis()));
    ...
    public String getHelpText() {
        try {
            return FileManager.readInputStream(TALEAnalysisTool.class.getClassLoader()
                  .getResourceAsStream("annotale/tools/RenameTool.txt")).toString();
        } catch (IOException e) {
            e.printStackTrace();
            return "";
        }
