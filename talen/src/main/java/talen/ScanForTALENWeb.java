...
import talen.FastTALENScanner.FastTALENScannerParameterSet;
import talen.FastTALENScanner.Output;
import talen.GFFParser.GFFList;
import tals.ScanForTBSWeb;
import tals.TALgetter13DiffSM;
...
public class ScanForTALENWeb {
	...
	public static void main( String[] args ) throws Exception {
		...
		ga.setHelp( FileManager.readInputStream( ScanForTBSWeb.class.getClassLoader().getResourceAsStream( "talen/helpoffer.txt" ) ).toString() );
		...
		if(((SelectionParameter)tempPars.getParameterForName( "Model" )).getSelected() == 1){
			...
			if(mod2.getValue().equals( "TALgetter" )){
				model = (TALgetterDiffSM)XMLParser.extractObjectForTags( FileManager.readInputStream( ScanForTALENWeb.class.getClassLoader().getResourceAsStream( "tals/talfinder_obg2_hyp_bg.xml" ) ), "model" );		
			}else{
				model = (TALgetterDiffSM)XMLParser.extractObjectForTags( FileManager.readInputStream( ScanForTALENWeb.class.getClassLoader().getResourceAsStream( "tals/talfinder_obg2_hyp_bg_map.xml" ) ), "model" );
			}
		}
