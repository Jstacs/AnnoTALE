...
import annotale.Tools.ProteinAlphabetContainer;
...
public class ClassBuilderTool implements JstacsTool {
    ...
    public ToolParameterSet getToolParameters() {
        ...
        try {
            ...
            SimpleParameter cut = new SimpleParameter(DataType.DOUBLE, "Cutoff",
                  "Cutoff value defining the maximum distance of a TALE class", true,
                  new NumberValidator<Comparable<Double>>(0.0, Double.MAX_VALUE), 5.0);

            SimpleParameter pval = new SimpleParameter(DataType.DOUBLE, "Significance level",
                  "Cutoff value on the p-value representing alignment significance", true,
                  new NumberValidator<Comparable<Double>>(0.0, 1.0), 0.01);
    ...
    public static TALE[] readProteinTALEs(FileRepresentation fr, Protocol protocol)
          throws Exception {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    ds = new DataSet(DNAAlphabetContainer.SINGLETON,
                          new SparseStringExtractor(br, '>', "",
                                new SimpleSequenceAnnotationParser()));
                } catch (Exception ex) {
                    BufferedReader br = new BufferedReader(new StringReader(content));
                    ds = new DataSet(ProteinAlphabetContainer.SINGLETON,
                          new SparseStringExtractor(br, '>', "",
                                new SimpleSequenceAnnotationParser()));
                }
                ...
                for (int i = 0; i < ds.getNumberOfElements(); i++) {
                    String id = ds.getElementAt(i)
                          .getSequenceAnnotationByType("unparsed comment line", 0).getResultAt(0)
                          .getValue().toString();
                    protocol.append(id + "\n");
                    ...
                    if (prot != null && prot.getNumberOfRepeats() > 0) {
                        ...
                        protocol.appendWarning("Removed putative pseudo gene " + prot.getId()
                              + ", because it has zero repeats.\n");
                        //System.err.println( "Removed putative pseudo gene "+prot.getId()+"." );
                    } else {
                        protocol.appendWarning("Removed putative pseudo gene " + id
                ...
                BufferedReader br = new BufferedReader(new StringReader(content));
                DataSet ds = new DataSet(RVDAlphabetContainer.SINGLETON,
                ...
                protocol.appendWarning(
                      "Loaded TALEs from RVD sequences. Some properties of TALEs (aberrant repeats, codon mismatches) cannot be displayed due to lacking information.\n");
                return tales;
            }
        }
    }
    ...
    public ToolResult run(ToolParameterSet parameters, Protocol protocol, ProgressUpdater progress,
        ...
        ress[0] = new TextResult("Class builder", "TALE class builder definition",
              new FileRepresentation("", res.getFirstElement().toXML().toString()), "xml",
              "TALE Class Builder", TALEFamilyBuilder.class.getName(), true);
        ress[1] = new PlotGeneratorResult("Tree of classes", "The tree of class similarities",
        ...
        for (int i = 0; i < famRes.length; i++) {
            ...
            PlotGeneratorResult pgr = new PlotGeneratorResult("Class tree for " + fam.getFamilyId(),
                  "Plot of the tree of the TALEs in this class", fam, true);

            //fam.plotFamilyToFile(outpath+"/family_"+(i+1), adaptor );
            String report = famRes[i].toString(model, res.getFirstElement());

            TextResult fileres = new TextResult("Class report for " + fam.getFamilyId(),
                  "Report for class " + fam.getFamilyId(), new FileRepresentation("", report),
                  "txt", "TALE Class Builder", null, false);

            ResultSetResult rsr = new ResultSetResult("Class " + fam.getFamilyId(),
                  "Collection of results for class " + fam.getFamilyId(), null,
    ...
    public String getHelpText() {
        try {
            return FileManager.readInputStream(ClassBuilderTool.class.getClassLoader()
                  .getResourceAsStream("annotale/tools/ClassBuilderTool.txt")).toString();
        } catch (IOException e) {
            e.printStackTrace();
            return "";
        }
