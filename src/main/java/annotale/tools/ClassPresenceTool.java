...
public class ClassPresenceTool implements JstacsTool {
    ...
    public ToolParameterSet getToolParameters() {
        ...
        FileParameter exclude = new FileParameter("Excluded strains",
              "List of the names of strains to be excluded", "txt", true);

        FileParameter include = new FileParameter("Included strains",
              "List of the names of strains to be included", "txt", true);

        try {
            SimpleParameter cut = new SimpleParameter(DataType.DOUBLE, "Cut height",
                  "The height at which the class tree is cut", true,
                  new NumberValidator<Double>(0.0, Double.MAX_VALUE), 10.0);

            SelectionParameter sp = new SelectionParameter(DataType.PARAMETERSET,
                  new String[]{"by class tree", "alphabetically"},
                  new ParameterSet[]{new SimpleParameterSet(cut), new SimpleParameterSet()},
                  "Arrangement of classes",
                  "If the list of classes should be arranged by the order in the class tree or alphabetically",
                  true);

            SelectionParameter inex = new SelectionParameter(DataType.PARAMETERSET,
                  new String[]{"none", "include", "exclude"},
                  new ParameterSet[]{new SimpleParameterSet(), new SimpleParameterSet(
                        include, new SimpleParameter(DataType.BOOLEAN, "Keep order in plots",
                        "If selected, order of strains in the plot will be kept as in the include file",
                        true, false)
                  ), new SimpleParameterSet(exclude)}, "Exclude/Include",
                  "Choose wether to exclude or (exclusively) include specific strains", true);

            SimpleParameter bool = new SimpleParameter(DataType.BOOLEAN, "Strains by presence",
                  "If selected, strains will be ordered by presence, otherwise by TALE distance",
                  true, false);
    ...
    public ToolResult run(ToolParameterSet parameters, Protocol protocol, ProgressUpdater progress,
        ...
        if (((SelectionParameter) parameters.getParameterAt(1)).getSelected() == 0) {
            height2 = (Double) ((ParameterSet) parameters.getParameterAt(1)
                  .getValue()).getParameterAt(0).getValue();
        }
        ...
        if (inex.getSelected() == 2) {
            ...
            if (((ParameterSet) inex.getValue()).getParameterAt(0).getValue() != null) {
                String cont = ((FileParameter) ((ParameterSet) inex.getValue()).getParameterAt(
            ...
            if (((ParameterSet) inex.getValue()).getParameterAt(0).getValue() != null) {
                ...
                boolean keepOrder = (Boolean) ((SimpleParameter) ((ParameterSet) inex.getValue()).getParameterAt(
                      1)).getValue();
                if (keepOrder) {
        ...
        for (int i = 0; i < tales.length; i++) {
            if ((excluded == null && included == null) || (excluded != null && !excluded.contains(
                  tales[i].getStrain())) || (included != null && included.contains(
                  tales[i].getStrain()))) {
        ...
        if (bool) {
                ...
                public double getDistance(String o1, String o2) throws Exception {
                    ...
                    for (int i = 0; i < present2.length; i++) {
                        for (int j = 0; j < present2[i].length; j++) {
                            if (present2[i][j][tempMap.get(o1)] != present2[i][j][tempMap.get(
                                  o2)]) {
                                dist++;
                            }
                        }
            ...
            metric = new DistanceMetric<String>() {
                ...
                public double getDistance(String o1, String o2) throws Exception {
                    ...
                    for (int i = 0; i < presTALEs.length; i++) {
                        for (int j = 0; j < presTALEs[i].length; j++) {
                            ...
                            if (tal1 != null && tal2 != null) {
                                contrib = TALEAligner.align(tal1, tal2, builder.getCosts(),
                                      builder.getAlignmentType(), builder.getExtraGapOpening(),
                                      builder.getExtraGapExtension()).getCost();
                            } else if (tal1 == null && tal2 == null) {
                                contrib = 0;
                            }
                 ...
                 * @return
                 */
                private TALE findBestMatch(TALE[][][] presTALEs, HashMap<String, Integer> tempMap,
                      String o1,
                    ...
                    for (int i = 0; i < presTALEs.length; i++) {
                        for (int j = 0; j < presTALEs[i].length; j++) {
                            ...
                            if (presTALEs[i][j][k1] != null && presTALEs[i][j][k2] == null) {
                                double temp = TALEAligner.align(origin, presTALEs[i][j][k1],
                                            builder.getCosts(), builder.getAlignmentType(),
                                            builder.getExtraGapOpening(), builder.getExtraGapExtension())
                                      .getCost();
                                if (temp < minDist) {
                                    temp = minDist;
        ...
        TextResult tr = new TextResult("Strain tree",
              "Strain tree (average linkage) in newick format",
              new FileParameter.FileRepresentation("", newick), "txt", this.getToolName(), null,
              true);
        ...
        ClassPresencePlotter cpp = new ClassPresencePlotter(subtrees, present, strains, height,
        ...
        return new ToolResult("Result of " + getToolName(), "", null, rs, parameters, getToolName(),
    ...
    private ListResult getList(double[][] distMat, String[] strains) {
        ...
        return new ListResult("Distance matrix", "Distance matrix of strains", null,
              ress.toArray(new ResultSet[0]));
    }

    private ListResult getList(ClusterTree<TALEFamily>[] subtrees, boolean[][][] present,
          String[] strains) {
            ...
            for (int k = 0; k < present[i].length; k++) {
                ...
                if (isTrue) {
                    ...
                    for (int j = 0; j < strains.length; j++) {
                        res.add(
                              new CategoricalResult(strains[j], "", present[i][k][j] ? "y" : "n"));
                    }
    ...
    public String getHelpText() {
        try {
            return FileManager.readInputStream(TALEAnalysisTool.class.getClassLoader()
                  .getResourceAsStream("annotale/tools/ClassPresenceTool.txt")).toString();
        } catch (IOException e) {
            e.printStackTrace();
            return "";
        }
    ...
    public static class ClassPresencePlotter implements PlotGenerator {
        ...
        public ClassPresencePlotter(ClusterTree<TALEFamily>[] subtrees, boolean[][][] present,
              String[] strains, double maxDist, boolean drawTrees) {
        ...
        public void generatePlot(GraphicsAdaptor ga) throws Exception {
            ...
            for (int i = 0; i < strains.length; i++) {
                ...
                drawRotate(graphics,
                      treeWidth + w + lineHeight * 2 * (i) + lineHeight * 1.5 + lineHeight / 2,
                      yoff - fontDim[1] / 2, -50, strains[i]);
                graphics.drawLine(treeWidth + w + lineHeight * 2 * (i) + lineHeight / 2, yoff,
                      treeWidth + w + lineHeight * 2 * (i) + lineHeight / 2,
                      dim[1] - 4 * lineHeight);
            }
            graphics.drawLine(treeWidth + w + lineHeight * 2 * strains.length + lineHeight / 2,
                  yoff, treeWidth + w + lineHeight * 2 * strains.length + lineHeight / 2,
                  dim[1] - 4 * lineHeight);
            ...
            for (double i = min; i <= max + tick / 2.0; i += tick) {
                ...
                if (x > lineHeight / 2) {
                    ...
                    int textCenter = (int) Math.round(
                          graphics.getFontMetrics().getStringBounds(label, graphics).getCenterX());
        ...
        public int[] getDimension(Graphics2D graphics, ClusterTree<TALEFamily>[] trees,
              String[] strains) {
            ...
            int tw = treeWidth + (int) (idDim[0] * 1.2) + (strains.length + 1) * lineHeight * 2
                  + (int) (fh * 0.6);
        ...
        private int plotSubtree(Graphics2D graphics, ClusterTree<TALEFamily> subtree,
              boolean[][] present, int yoff, int idWidth, int idHeight, boolean drawTree) {
            ...
            for (int i = 0; i < present.length; i++) {
                ...
                if (isTrue) {
                    graphics.drawString(fams[i].getFamilyId(),
                          xoff - idWidth + (int) (lineHeight * 0.75),
                          yoff - (lineHeight * 2 - idHeight));
                    graphics.drawLine(xoff, yoff - 2 * lineHeight,
                          xoff + present[i].length * lineHeight * 2, yoff - 2 * lineHeight);
                    for (int j = 0; j < present[i].length; j++) {
                        if (present[i][j]) {
                            graphics.setColor(new Color(0f, 0f, 0.5f, 0.6f));
                            graphics.fillRect(xoff + j * lineHeight * 2 + 2,
                                  yoff - 2 * lineHeight + 2, 2 * lineHeight - 4,
                                  2 * lineHeight - 4);
                            graphics.setColor(Color.BLACK);
                        }
                    }
            ...
            graphics.drawLine(xoff, yoff - 2 * lineHeight,
                  xoff + present[0].length * lineHeight * 2, yoff - 2 * lineHeight);
        ...
        private int drawTree(Graphics2D graphics, ClusterTree<TALEFamily> subtree,
              boolean[][] present, int off, int yoff, int level) {
            ...
            if (n > 1) {
                ...
                int numRight = drawTree(graphics, right, present, off + left.getNumberOfElements(),
                      yoff + numLeft * 2 * lineHeight, level + 1);

                graphics.drawLine(xoff, yoff + numLeft * lineHeight, xoff,
                      yoff + numLeft * 2 * lineHeight + numRight * lineHeight);

                int xoff2 = treeWidth + lineHeight;
                if (numLeft > 1) {
                    xoff2 = lineHeight + (int) ((1.0 - left.getDistance() / maxDist) * treeWidth);
                }
                graphics.drawLine(xoff, yoff + numLeft * lineHeight, xoff2,
                      yoff + numLeft * lineHeight);
                ...
                graphics.drawLine(xoff, yoff + numLeft * 2 * lineHeight + numRight * lineHeight,
                      xoff2, yoff + numLeft * 2 * lineHeight + numRight * lineHeight);
