...
public class TALEAnalysisTool implements JstacsTool {
    ...
    public ToolParameterSet getToolParameters() {
        ...
        try {
            SimpleParameter name = new SimpleParameter(DataType.STRING, "Name",
                  "A name for this run of " + getToolName(), false);
            FileParameter fp = new FileParameter("TALE DNA sequences",
                  "The DNA sequences of the TALEs", "fasta,fa,fas,fna", true);
    ...
    public ToolResult run(ToolParameterSet parameters, Protocol protocol, ProgressUpdater progress,
        ...
        DataSet ds = new DataSet(DNAAlphabetContainer.SINGLETON,
        ...
        for (int i = 0; i < ds.getNumberOfElements(); i++) {
            String id = ds.getElementAt(i).getSequenceAnnotationByType("unparsed comment line", 0)
            ...
            if (dna != null) {
                ...
                for (int j = 0; j < dna.getNumberOfRepeats(); j++) {
                    dnaTALEs.append(
                          ">" + id + ": repeat " + (j + 1) + "\n" + dna.getRepeat(j).getRepeat()
                                .toString() + "\n");
                }
                dnaTALEs.append(">" + id + ": C-terminus\n" + dna.getEnd().toString() + "\n");
            } else {
                protocol.appendWarning(
                      "TALE " + id + " could not be analyzed, splitting into regions failed.\n");
            }
            ...
            if (prot != null) {
                ...
                if (prot.containsAberrantRepeat()) {
                    ...
                    for (int j = 0; j < prot.getNumberOfRepeats() - 1; j++) {
                        if (prot.getRepeat(j).getType() == Type.LONG
                              || prot.getRepeat(j).getType() == Type.SHORT) {
                ...
                protocol.appendWarning("Protein version of TALE " + id
                      + " could not be analyzed, splitting into regions failed.\n");
            }
        }
        ...
        TextResult fr1 = new TextResult("TALE DNA parts" + names,
              "Parts of the TALE (N-/C-terminus, repeats) as DNA sequences",
              new FileRepresentation("", dnaTALEs.toString()), "fasta", "TALE Analysis",
              "fasta/dna", true);
        TextResult fr2 = new TextResult("TALE Protein parts" + names,
              "Parts of the TALE (N-/C-terminus, repeats) as protein sequences",
              new FileRepresentation("", protTALEs.toString()), "fasta", "TALE Analysis",
              "fasta/as", true);
        TextResult fr3 = new TextResult("TALE RVDs" + names, "Sequence of RVDs of the TALEs",
              new FileRepresentation("", rvdTALEs.toString()), "fasta", "TALE Analysis",
              "fasta/rvd", true);

        ResultSet set = new ResultSet(new Result[]{fr1, fr2, fr3});
        return new ToolResult("Result of " + getToolName() + names,
    ...
    public static TALE[] parseTALEsFromParts(String content, Protocol protocol) throws Exception {
        ...
        for (int i = 0; i < ds.getNumberOfElements(); i++) {
            ...
            String name = seq.getSequenceAnnotationByType("unparsed comment line", 0).getResultAt(0)
                  .getValue().toString();
            ...
            if (lastid != null && !lastid.equals(id)) {
                if (repeats.size() > 0) {
                    ...
                    if (ds.getAlphabetContainer()
                          .checkConsistency(DNAAlphabetContainer.SINGLETON)) {
                        try {
                            t = t.getTranslatedTALE(Translator.DEFAULT);
                        } catch (Exception e) {
                            protocol.appendWarning(
                                  "Could not translate TALE " + t.getId() + ". Reason:\n"
                                        + e.getMessage() + "\n");
                            t = null;
                        }
                    }
                    ...
                    protocol.appendWarning(
                          "Removed TALE " + lastid + ", because it has zero repeats.\n");
                }
                nterm = null;
                cterm = null;
                repeats.clear();
    ...
    public String getHelpText() {
        try {
            return FileManager.readInputStream(TALEAnalysisTool.class.getClassLoader()
                  .getResourceAsStream("annotale/tools/TALEAnalysisTool.txt")).toString();
        } catch (IOException e) {
            e.printStackTrace();
            return "";
        }
