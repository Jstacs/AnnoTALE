		...
		/*	this.costs = costs;
            ...
            for (int j = 0; j < members.length; j++) {
                for (int k = 0; k < members.length; k++) {
                    if (j != k) {
                        StringAlignment sa = TALEAligner.align(members[j], members[k],
                              builder.costs, builder.at, builder.extraGapOpening,
                              builder.extraGapExtension);
                        alignments[j][k] = sa;
                    }
                }
            }
...
/*			XMLParser.appendObjectWithTags( xml, at, "at" );
        ...
        public String getSpecificityConsensus(TALgetterDiffSM model)
        ...
        public double[][] getSpecificityProfile(TALgetterDiffSM model)
              throws IllegalArgumentException, WrongAlphabetException {
                ...
                for (int j = 0; j < parts.length; j++) {
                    ...
                    if (parts[j].equals("--") || !modelCon.isSymbol(parts[j])) {
                        ...
                        for (int k = 0; k < temp[j + 1].length; k++) {
                            temp[j + 1][k] =
                                  temp[j + 1][k] * pair.getSecondElement()[0] + 0.25 * (1.0
                                        - pair.getSecondElement()[0]);
                        }
        ...
        public double[] getDistance(TALE tale, FamilyDistance dist, TALEFamilyBuilder builder) {
            ...
            for (int i = 0; i < members.length; i++) {
                ds[i] = TALEAligner.align(tale, members[i], builder.costs, builder.at,
                      builder.extraGapOpening, builder.extraGapExtension).getCost();
            }

            int idx = ToolBox.getMinIndex(ds);
            double minD = ds[idx] / (double) Math.min(tale.getNumberOfRepeats(),
                  members[idx].getNumberOfRepeats());
        ...
        public double getSignificance(TALE tale, AlignmentPValues pv, FamilyDistance dist,
            ...
            double[] ds = new double[members.length];
            for (int i = 0; i < members.length; i++) {
                double temp = TALEAligner.align(tale, members[i], builder.costs, builder.at,
                      builder.extraGapOpening, builder.extraGapExtension).getCost();
                ds[i] = pv.getLog10PValue(tale, members[i], temp, builder.extraGapOpening,
                      builder.extraGapExtension);
            }
            if (dist == FamilyDistance.MAX) {
        ...
        public double getFamilySignificance(AlignmentPValues pv, TALEFamilyBuilder builder) {
            ...
            for (int i = 1; i < alignments.length; i++) {
                for (int j = 0; j < i; j++) {
                    double p = pv.getLog10PValue(members[i], members[j], alignments[i][j].getCost(),
                          builder.extraGapOpening, builder.extraGapExtension);
        ...
        public String toString(TALgetterDiffSM model, TALEFamilyBuilder builder) {
            StringBuffer sb = new StringBuffer();
            sb.append("Class " + id + " for (" + builder.costs.getClass().getSimpleName() + ", "
                  + builder.cut + ", " + builder.at + ")\n");
            ...
            if (builder.costs instanceof AffineCosts) {
                ...
                if (c2 instanceof RVDCosts) {
                    RVDCosts c3 = (RVDCosts) c2;
                    double p = this.getFamilySignificance(
                          new AlignmentPValues(builder.getAllTALEs(), c3), builder);
            ...
            for (int i = 1; i < members.length; i++) {
                for (int j = 0; j < i; j++) {
                    sb.append(
                          members[i].getId() + " vs. " + members[j].getId() + ": " + format.format(
                                alignments[i][j].getCost()) + "\n");
                }
            }
        ...
        private TALEFamily removeTALE(double[][] dmat, LinkedList<ClusterTree<TALE>> tales,
              int indexOff, TALEFamilyBuilder builder) {
            ...
            if (list.size() == 0) {
                ...
                ClusterTree<TALE> newTree = hclust.cluster(indexOff, dmat,
                      list.toArray(new ClusterTree[0]));//TODO efficiency
        ...
        private TALEFamily addTALE(double[][] dmat, TALE tale, int newIndex, int indexOff,
              TALEFamilyBuilder builder) {
        ...
        private Pair<TALE[], String[]> getInducedMultipleAlignment2(ClusterTree<TALE> tree,
              TALEFamily family) {
            TALE[] members = tree.getClusterElements();
            if (members.length == 1) {
                Sequence rvds = members[0].getRvdSequence();
                return new Pair<TALE[], String[]>(new TALE[]{members[0]},
                      new String[]{" " + rvds.toString()});
            } else if (members.length == 2) {
                //System.out.println("searching alignment for IDs "+members[0].getId()+","+members[1].getId());
                StringAlignment al = family.getAlignmentForIDs(members[0].getId(),
                      members[1].getId());
                return new Pair<TALE[], String[]>(new TALE[]{members[0], members[1]},
                      new String[]{al.getAlignedString(0), al.getAlignedString(1)});
            } else {
                ...
                return new Pair<TALE[], String[]>(resTales.toArray(new TALE[0]),
                      resAls.toArray(new String[0]));
        ...
        private Pair<TALE[], String[]> getInducedMultipleAlignment(ClusterTree<TALE> tree,
              TALEFamily family) {
            TALE[] members = tree.getClusterElements();
            if (members.length == 1) {
                Sequence rvds = members[0].getRvdSequence();
                ...
                for (int i = 1; i < subs.length; i++) {
                    ...
                    while (idx1 >= 0 || idx2 >= 0) {
                        if (idx1 >= 0 && idx2 >= 0 && prevAl.charAt(idx1) == al2.charAt(
                              idx2)) {//indentical
                            idx1--;
                            idx2--;
                        } else if (idx1 >= 0
                              && prevAl.charAt(idx1) == '-') {//we had a gap that we don't have now
                            addCurr[idx2 + 1]++;
                            idx1--;
                ...
                for (int i = 0; i < onlyGaps.length; i++) {
                    int j = 0;
                    while (tempAls.get(i).charAt(j) == '-') {
                        onlyGaps[i][j] = true;
                        j++;
                    }
                    j = tempAls.get(i).length() - 1;
                    while (tempAls.get(i).charAt(j) == '-') {
                        onlyGapsEnd[i][j] = true;
                        j--;
                    }
                }
                for (int i = 0; i < onlyGaps[0].length; i++) {
                    int j = 0;
                    while (j < tempAls.size() && tempAls.get(j).charAt(i) == '-'
                          && !onlyGaps[j][i]) {
                        j++;
                    }
                    ...
                    j = tempAls.size() - 1;
                    while (j >= 0 && tempAls.get(j).charAt(i) == '-' && !onlyGaps[j][i]) {
                        j--;
                    }
                    if (j > 0 && j < tempAls.size() && tempAls.get(j).charAt(i) == '-') {
    ...
    public TALEFamilyBuilder(TALE[] tales)
          throws IllegalArgumentException, IOException, WrongAlphabetException {
        this(tales, new AffineCosts(5.0, new RVDCosts(1.0, 0.2, 0.8, 0.0)), Linkage.AVERAGE,
              AlignmentType.SEMI_GLOBAL, 1.0, 0.1, 5.0, 0.01);
    }

    public TALEFamilyBuilder(TALE[] tales, Costs costs, Linkage linkage, AlignmentType at,
        ...
        Pair<double[][], ClusterTree<TALE>> pair = cluster(tales, linkage, costs, at,
              extraGapOpening, extraGapExtension);
...
/*	public ClusterTree<TALE> getFamilyTree(){
	...
	*/
    ...
    public ClusterTree<TALEFamily> clusterFamilies() {
        ...
        ClusterTree<TALEFamily> famTree = superCluster.dropBelow(familyRootOriginalIndexes,
              families);
    ...
    private static double[][] computeDistMatrix(TALE[] tales, Costs costs, AlignmentType at,
          double extraGapOpening, double extraGapExtension) {
        double[][] dmat = new double[tales.length][tales.length];

        for (int j = 0; j < tales.length; j++) {
            for (int k = 0; k < tales.length; k++) {
                double sc = TALEAligner.align(tales[j], tales[k], costs, at, extraGapOpening,
                      extraGapExtension).getCost();
                dmat[j][k] = sc;
            }
    ...
    private static double[][] computeDistMatrix2(TALE[] tales, Costs costs, AlignmentType at,
          double extraGapOpening, double extraGapExtension) {
        double[][] dmat = new double[tales.length][tales.length];
    ...
    private double[][] computeDistMatrix3(TALE[] tales, int firstNew, Costs costs, AlignmentType at,
          double extraGapOpening, double extraGapExtension) {
        double[][] dmat = new double[tales.length][tales.length];
    ...
    private static Pair<double[][], ClusterTree<TALE>> cluster(TALE[] tales, Linkage linkage,
          Costs costs, AlignmentType at, double extraGapOpening, double extraGapExtension) {
    ...
    public void splitClass(String className) {
        ...
        dmat = computeDistMatrix(allTALEs.toArray(new TALE[0]), costs, at, extraGapOpening,
              extraGapExtension);
    ...
    public void removeTALEsFromFamilies(LinkedList<TALE> toRemove) {
        ...
        double[][] newDmat = computeDistMatrix(remain, costs, at, extraGapOpening,
              extraGapExtension);//TODO efficiency
    ...
    public void addTALEsToFamilies(Pair<Integer, LinkedList<TALE>>[] assignment, TALE[] unassigned,
          FamilyIdGenerator idGenerator) {
        ...
        if (maxIndex + 1 != dmat.length) {
            throw new RuntimeException(
                  "Indexes do not match " + (maxIndex + 1) + " <-> " + dmat.length);
        }
        ...
        double[][] newDmat = computeDistMatrix3(allTALEs, maxIndex + 1, costs, at, extraGapOpening,
              extraGapExtension);//TODO efficiency
        ...
        for (int i = 0; i < assignment.length; i++) {
            ...
            for (int j = 0; j < newTALEs.size(); j++, k++) {
                newFams[famIdx] = newFams[famIdx].addTALE(newDmat, newTALEs.get(j), k, -minIndex,
                      this);
        ...
        if (unassigned.length > 0) {
            ...
            for (int i = 0; i < createdTrees.length; i++) {
                ...
                if (d / (double) l >= RELATIVE_MISMATCH_SHORT) {
                    ClusterTree<TALE>[] subs = Hclust.cutTree(RELATIVE_MISMATCH_SHORT * l,
                          createdTrees[i]);
    ...
    public Pair<TALEFamily, Double> getMostSignificantFamily(TALE tale, AlignmentPValues pv,
          FamilyDistance dist) {
        Pair<Integer, Double> pair = getMostSignificantFamilyIndex(tale, pv, dist);
        return new Pair<TALEFamily, Double>(getFamily(pair.getFirstElement()),
              pair.getSecondElement());
    }

    public Pair<Integer, Double> getMostSignificantFamilyIndex(TALE tale, AlignmentPValues pv,
          FamilyDistance dist) {
        int closest = -1;
        double min = Double.POSITIVE_INFINITY;
        for (int i = 0; i < this.families.length; i++) {
