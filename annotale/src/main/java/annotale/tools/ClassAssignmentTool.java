...
import tals.ScanForTBSCLI;
import tals.TALgetterDiffSM;
...
public class ClassAssignmentTool implements JstacsTool {
                ...
                if (famId == null) {
                    ...
                    for (int j = 0; j < mems.length; j++) {
                        ...
                        if (!mems[j].getId().contains("tempTALE")) {
                            tid += " (" + mems[j].getId() + ")";
                        } else if (mems[j].getId().contains("Pseudo") || mems[j].getId()
                              .contains("pseudo")) {
                            tid += " (Pseudo)";
                        }
                        mems[j].setId(tid);
    ...
    public ToolParameterSet getToolParameters() {
        ...
        FileParameter fp = new FileParameter("TALE sequences",
              "The sequences of the TALEs (DNA or protein), or \"TALE DNA parts\" or \"TALE Protein parts\" output of \"TALE Analysis\", or RVD sequences.",
              "fasta,fa,fas", true);
        ...
        try {
            ...
            ap = new SimpleParameter(DataType.STRING, "Accession",
                  "The accesion number of the genome (if applicable).", false);
    ...
    @Override
    public ToolResult run(ToolParameterSet parameters, Protocol protocol, ProgressUpdater progress,
        ...
        try {
            newttales = ClassBuilderTool.readProteinTALEs(
                  ((FileParameter) parameters.getParameterAt(1)).getFileContents(), protocol);
        } catch (Exception e) {
            protocol.appendWarning(
                  "TALE sequence could not be read. Please make sure that the input is indeed a FastA containing TALE sequences or TALE parts.\n");
        ...
        for (int i = 0; i < newttales.length; i++) {
            ...
            if (newttales[i].getNumberOfRepeats() > 3) {
                ...
                if (idx.getSecondElement()
                      < cut/* && idx.getSecondElement()/(double)newttales[i].getNumberOfRepeats()<TALEFamilyBuilder.RELATIVE_MISMATCH_SHORT && idx.getSecondElement()/(double)fams[idx.getFirstElement()].getLengthOfLongestFamilyMember() < TALEFamilyBuilder.RELATIVE_MISMATCH_SHORT*/) {
                    report.append(
                          "Assigned to class " + fams[idx.getFirstElement()].getFamilyId() + ".\n");
                    double p2 = fams[idx.getFirstElement()].getSignificance(newttales[i], null,
                          null, builder);
                    report.append("distance " + format.format(idx.getSecondElement()) + ", (p="
                          + formatE.format(Math.pow(10, p2)) + ")\n");
                ...
                for (int j = 0; j < fams.length; j++) {
                    if (j != idx.getFirstElement() || !assigned) {
                        ...
                        if (p < Math.log10(0.001)) {
                            ...
                            report.append(
                                  "Class " + fams[j].getFamilyId() + ": distance " + format.format(
                                        d) + ", (p=" + formatE.format(Math.pow(10, p)) + "),\n");
                            found = true;
                        }
                    }
                }
                ...
                ress.add(new TextResult("Report for " + newttales[i].getId(), "",
                      new FileRepresentation("", report.toString()), "txt", "TALE Class Assignment",
                      null, false));

            } else {
                report.append("Not assigned to a class, because it has less than 4 repeats.\n");
                protocol.appendWarning("TALE " + newttales[i].getId()
                      + " not assigned to a class, because it has less than 4 repeats.\n");
        ...
        topLevel.add(new ResultSetResult("Reports", "Assignment reports for new TALEs", null,
              new ResultSet(ress.toArray(new Result[0]))));
        ...
        for (int i = 0; i < added.length; i++) {
            ces[i] = new ComparableElement<Boolean, TALEFamilyBuilder.TALEFamily>(added[i],
                  newFams[i]);
        }
        Arrays.sort(ces);
        for (int i = 0; i < added.length; i++) {
        ...
        TALgetterDiffSM model = (TALgetterDiffSM) XMLParser.extractObjectForTags(
              FileManager.readInputStream(ScanForTBSCLI.class.getClassLoader()
                    .getResourceAsStream("annotale/talfinder_obg2_hyp_bg.xml")), "model");

        for (int i = 0; i < added.length; i++) {
            if (added[i]) {
                PlotGeneratorResult pgr = new PlotGeneratorResult(
                      "Class tree for " + newFams[i].getFamilyId(),
                      "Plot of the tree of the TALEs in this class", newFams[i], true);
                TextResult fileres = new TextResult("Class report for " + newFams[i].getFamilyId(),
                      "Report for class " + newFams[i].getFamilyId(),
                      new FileRepresentation("", newFams[i].toString(model, builder)), "txt",
                      "TALE Class Assignment", null, false);
                ResultSetResult rsr = new ResultSetResult(
                      "Modified class " + newFams[i].getFamilyId(),
                      "Collection of results for class " + newFams[i].getFamilyId(), null,
                      new ResultSet(new Result[]{fileres, pgr}));
        ...
        if (notAssigned.size() > 0) {
            ...
            FamilyResult[] res2 = BuildFamilies.getFamilyResults(newFams, builder.getPVal(),
                  builder, added.length);
            Arrays.sort(res2);
            protocol.append("... found " + res2.length + " new classes.\n");
            for (int i = 0; i < res2.length; i++) {
                PlotGeneratorResult pgr = new PlotGeneratorResult(
                      "Class tree for " + res2[i].getFamily().getFamilyId(),
                      "Plot of the tree of the TALEs in this class", res2[i].getFamily(), true);
                TextResult fileres = new TextResult(
                      "Class report for " + res2[i].getFamily().getFamilyId(),
                      "Report for class " + res2[i].getFamily().getFamilyId(),
                      new FileRepresentation("", res2[i].getFamily().toString(model, builder)),
                      "txt", "TALE Class Assignment", null, false);
                ResultSetResult rsr = new ResultSetResult(
                      "New class " + res2[i].getFamily().getFamilyId(),
                      "Collection of results for class " + res2[i].getFamily().getFamilyId(), null,
                      new ResultSet(new Result[]{fileres, pgr}));
        ...
        topLevel.add(new ResultSetResult("Classes", "Modified and new TALE classes", null,
              new ResultSet(ress.toArray(new Result[0]))));
        ress.clear();
        ...
        if (gen != null) {
            ...
            summary.append(
                  "The " + map.length + " TALEs have been assigned to the following classes:\n");
            for (int i = 0; i < map.length; i++) {
                ...
                summary.append(
                      "- " + map[i][0] + " has been assigned to " + (num == 1 ? "new" : "existing")
                            + " class " + clazz + " with name " + map[i][1] + "\n");
            }
        }
        ...
        summary.append(
              "\n\nA detailed assignment report for each individual TALE can be found under \"Reports\".\n\n"
                    ...
                    + "Renamed TALE DNA and protein sequences are provided under \""
                    + "Renamed TALE protein sequences" + oss + "\".");

        topLevel.addFirst(
              new TextResult("Summary of class assignment", "A summary of the class assignment",
                    new FileRepresentation("", summary.toString()), "txt", "TALE Class Builder",
                    TALEFamilyBuilder.class.getName(), true));

        ListResult lr = new ListResult("TALE names" + oss,
              "Proposed TALE names based on class assignment", null,
              props.toArray(new ResultSet[0]));
        ...
        topLevel.add(
              new TextResult("Augmented class builder" + oss, "TALE class builder definition",
                    new FileRepresentation("", builder.toXML().toString()), "xml",
                    "TALE Class Builder", TALEFamilyBuilder.class.getName(), true));
        ...
        TextResult fres4 = new TextResult("Renamed TALE protein sequences" + oss,
              "The protein sequences of the assigned TALEs using the proposed names",
              new FileRepresentation("", prot.toString()), "fasta", "Class Assignment", "fasta/as",
              true);

        ResultSetResult seqs = null;

        if (dna.length() > 0) {
            TextResult fres3 = new TextResult("Renamed TALE DNA sequences" + oss,
                  "The DNA sequences of the assigned TALEs using the proposed names",
                  new FileRepresentation("", dna.toString()), "fasta", "Class Assignment",
                  "fasta/dna", true);
            seqs = new ResultSetResult("Renamed TALE sequences" + oss,
                  "The sequences of the assigned TALEs using the proposed names", null,
                  new ResultSet(new Result[]{fres3, fres4}));
        } else {
            seqs = new ResultSetResult("Renamed TALE sequences" + oss,
                  "The sequences of the assigned TALEs using the proposed names", null,
                  new ResultSet(new Result[]{fres4}));
        }
        ...
        return new ToolResult("Result of " + getToolName() + oss,
              getToolName() + " on " + ((FileParameter) parameters.getParameterAt(
                    1)).getFileContents().getFilename(), null, set, parameters, getToolName(),
    ...
    public String getHelpText() {
        try {
            return FileManager.readInputStream(ClassAssignmentTool.class.getClassLoader()
                  .getResourceAsStream("annotale/tools/ClassAssignmentTool.txt")).toString();
        } catch (IOException e) {
            e.printStackTrace();
            return "";
        }
