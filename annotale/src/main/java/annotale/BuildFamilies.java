...
public class BuildFamilies {
    ...
    public static class FamilyResult implements Comparable<FamilyResult> {
         ...
         */
        public FamilyResult(TALEFamily family, TALEFamily[] relatedFams, TALE[][] relatedTALEs,
              StringAlignment[][] alignments, double[][] pvals, int[][] myTALEs) {
            this.family = family;
            ...
            if (relatedFams.length == 0) {
                ...
                for (int i = 0; i < relatedFams.length; i++) {
                    ...
                    for (int j = 0; j < relatedTALEs[i].length; j++) {
                        sb.append(relatedTALEs[i][j].getId() + " related to "
                              + members[myTALEs[i][j]].getId() + " with score " + format.format(
                              alignments[i][j].getCost()) + " (p=" + formatE.format(
                              Math.pow(10, pvals[i][j])) + ")\n");
                    }
                    sb.append("\n");
                    sb.append("Alignments:\n");
                    for (int j = 0; j < alignments[i].length; j++) {
                        sb.append(
                              relatedTALEs[i][j].getId() + " vs. " + members[myTALEs[i][j]].getId()
                                    + ":\n");
    ...
    public static FamilyResult[] getFamilyResults(TALEFamily[] fams, double pval,
          TALEFamilyBuilder builder, int offset) {
        ...
        if (costs instanceof AffineCosts
              && ((AffineCosts) costs).getInternalCosts() instanceof RVDCosts) {

            pv = new AlignmentPValues(builder.getAllTALEs(),
                  (RVDCosts) ((AffineCosts) costs).getInternalCosts());
        }
        ...
        for (int i = offset; i < fams.length; i++) {
                ...
                if (i != j) {
                    ...
                    for (int k = 0; k < members.length; k++) {
                        for (int l = 0; l < members2.length; l++) {
                            ...
                            StringAlignment al = TALEAligner.align(members[k], members2[l], costs,
                                  at, extraGapOpening, extraGapExtension);
                            if (pv != null) {
                                double p = pv.getLog10PValue(members[k], members2[l], al.getCost(),
            ...
            ress[i - offset] = new FamilyResult(fams[i], relFams.toArray(new TALEFamily[0]),
                  relsTales.toArray(new TALE[0][0]), relsAls.toArray(new StringAlignment[0][0]),
                  relPs.toArray(new double[0][0]), relsMys.toArray(new int[0][0]));
    ...
    public static Pair<TALEFamilyBuilder, FamilyResult[]> build(TALE[] ttales, double cut,
          double pval) throws NonParsableException, IOException {
        RVDCosts rvdCosts = new RVDCosts(1.0, 0.2, 0.8, 0.0);//TODO
        Costs costs = new AffineCosts(5.0, 5.0, rvdCosts);

        //TALgetterDiffSM model = (TALgetterDiffSM)XMLParser.extractObjectForTags( FileManager.readInputStream( ScanForTBSCLI.class.getClassLoader().getResourceAsStream( "annotale/talfinder_obg2_hyp_bg.xml" ) ), "model" );

        //SpecificityCosts specCosts = new SpecificityCosts( 1.0, model );//TODO
        //Costs costs = new AffineCosts( 5.0, specCosts );

        TALEFamilyBuilder builder = new TALEFamilyBuilder(ttales, costs, linkage, at,
              extraGapOpening, extraGapExtension, cut, pval);
    ...
    public static void rename(TALE[] ttales) throws IOException {
        ...
        BufferedReader read = new BufferedReader(
              new FileReader("/Users/dev/Desktop/TAL-Chips/Genomes/proteins/tal_names.csv"));//TODO
    ...
    public static void plotGroups(GraphicsAdaptor adaptor, ClusterTree<TALEFamily> tree,
          String nameBase) throws IOException {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < famRes.length; i++) {
            ...
            adaptor.generateOutput(outpath + "/family_" + (i + 1) + "_specificity."
                  + adaptor.getGraphicsExtension());
        }
