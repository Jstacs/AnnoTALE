...
public class RVDCopies {
    ...
    private static void printCounts(Sequence[][] allSeqs2, String[] names, String suffix,
          String[] strains) {
                    ...
                    for (int l = 0; l < seqs2.length; l++) {
                        ...
                        for (int m = 0; m < matches.size(); m++) {
                            ...
                            System.out.println(names[i] + "\t" + names[k] + "\t"
                                  + ma.start1 + "\t" + ma.start2 + "\t" + (ma.len1 - (ma.start1
                                  + ma.lenm)) + "\t" + (ma.len2 - (ma.start2 + ma.lenm)) + "\t"
                                  + ma.lenm + "\t" + suffix + "\t" + strains[i] + "\t"
                                  + strains[k]);
                        }
    ...
    private static void findMatches(Sequence rvds1, Sequence rvds2, ArrayList<Match> matches,
          int minlen) {
        ...
        for (int i = 0; i < rvds1.getLength() - minlen + 1; i++) {
            ...
            for (int j = 0; j < rvds2.getLength() - minlen + 1; j++) {
                ...
                if (i == 0 || j == 0 || rvds1.discreteVal(i - 1) != rvds2.discreteVal(j - 1)) {
                    int k = 0;
                    while (k < sub1.getLength() && k < sub2.getLength()
                          && sub1.discreteVal(k) == sub2.discreteVal(k)) {
                        k++;
                    }
                    if (k > minlen) {
