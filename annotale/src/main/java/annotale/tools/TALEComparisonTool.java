...
public class TALEComparisonTool implements JstacsTool {
        ...
        public void generatePlot(GraphicsAdaptor ga) throws Exception {
            ...
            for (int i = 0; i < dist.length; i++) {
                for (int j = 0; j < dist[i].length; j++) {
                    if ((i < dist.length - 1 && j < dist[i].length - 1) || (i == dist.length - 1
                          && j == dist[i].length - 1)) {
                        Color c = Color.WHITE;
                        if (dist[i][j] > 0) {
                            float mix = (float) (dist[i][j] / maxDist);
                            c = new Color(Color.YELLOW.getRed() / 255f * (1f - mix) + 1f * (mix),
                                  Color.YELLOW.getGreen() / 255f * (1f - mix),
                                  Color.YELLOW.getBlue() / 255f * (1f - mix));
                        }
                        g.setColor(c);
                        g.fillRect(offx + j * repHeight, offy + i * repHeight, repHeight,
                              repHeight);
                        g.setColor(Color.BLACK);
                    } else {
            ...
            for (int i = 0; i < t2.getNumberOfRepeats(); i++) {
                String str = (i + 1) + "";
                Rectangle2D dim = g.getFontMetrics().getStringBounds(str, g);
                g.drawString(str, offx + i * repHeight + (repHeight - (int) dim.getWidth()) / 2,
                      margMain + margTop - margTop / 10);
            ...
            for (int i = 1; i < maxDist; i++) {
                ...
                Color c = new Color(Color.YELLOW.getRed() / 255f * (1f - mix) + 1f * (mix),
                      Color.YELLOW.getGreen() / 255f * (1f - mix),
                      Color.YELLOW.getBlue() / 255f * (1f - mix));
                g.setColor(c);
                g.fillRect(margLeft, repHeight / 2 + (int) ((maxDist - i - 1) * step), repHeight,
                      step);
                g.setColor(Color.BLACK);
            }
            g.drawLine((int) (margLeft * 0.9), repHeight / 2 + step / 2, margLeft - 1,
                  repHeight / 2 + step / 2);
            Rectangle2D dim = g.getFontMetrics().getStringBounds(((int) maxDist) + "", g);
            g.drawString(((int) maxDist) + "", (int) (margLeft * 0.8 - dim.getWidth()),
                  (int) (repHeight / 2 + step / 2 + dim.getHeight() / 2));

            g.drawLine((int) (margLeft * 0.9), (int) (repHeight / 2 + maxDist * step - step / 2),
                  margLeft - 1, (int) (repHeight / 2 + maxDist * step - step / 2));
            dim = g.getFontMetrics().getStringBounds("0", g);
            g.drawString("0", (int) (margLeft * 0.8 - dim.getWidth()),
                  (int) (repHeight / 2 + maxDist * step - step / 2 + dim.getHeight() / 2));

            g.drawLine(margLeft - 1, repHeight / 2 + step / 2, margLeft - 1,
                  (int) (repHeight / 2 + maxDist * step - step / 2));
            ...
            for (int i = 0; i < temp.length; i++) {
                ...
                g.drawString(temp[i], margLeft + repHeight
                            + (float) (width - dim.getWidth() - margLeft - 2 * repHeight) / 2f,
                      (float) (sumHeight * scale * 0.9 / temp.length * (i + 1)));
            }
    ...
    public ToolParameterSet getToolParameters() {
        ...
        FileParameter tales = new FileParameter("TALE sequences",
              "TALE sequences, either as complete DNA or AS sequences (e.g., output of TALE Prediction).",
              "fasta,fa,fas", true);
    ...
    public ToolResult run(ToolParameterSet parameters, Protocol protocol, ProgressUpdater progress,
        ...
        TALE[] input = ClassBuilderTool.readProteinTALEs(
              ((FileParameter) parameters.getParameterAt(0)).getFileContents(), protocol);
        ...
        return new ToolResult("Result of " + getToolName(),
              getToolName() + " on \"" + ((FileParameter) parameters.getParameterAt(
                    0)).getFileContents().getFilename() + "\"", null, set, parameters,
    ...
    private PlotGeneratorResult compareTALEs(TALE tal1, TALE tal2) {
        ...
        return new PlotGeneratorResult(
              "Repeat difference of " + tal1.getId() + (tal1.getId().equals(tal2.getId()) ? ""
                    : " and " + tal2.getId()), "Pairwise edit distance of TALE repeats", gen, true);
    ...
    public String getHelpText() {
        try {
            return FileManager.readInputStream(TALEAnalysisTool.class.getClassLoader()
                  .getResourceAsStream("annotale/tools/TALEComparisonTool.txt")).toString();
        } catch (IOException e) {
            e.printStackTrace();
            return "";
        }
