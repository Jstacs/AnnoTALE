...
import talen.InfixMatchFinder;
import talen.MatchFinder;
import talen.MatchFinder.Match;
import tals.ScanForTBSCLI;
import tals.TALgetterDiffSM;
import tals.TBSScanner;
...
public class PredictAndIntersectTargetsTool implements JstacsTool {
    ...
    public ToolParameterSet getToolParameters() {
        FileParameter genome = new FileParameter("Input sequences",
        ...
        FileParameter tales = new FileParameter("TALE sequences",
              "TALE sequences, either as complete DNA or AS sequences (e.g., output of TALE Prediction) or as RVD sequences.",
              "fasta,fa,fas", true);

        SelectionParameter selPar = null;
        try {
            selPar = new SelectionParameter(DataType.PARAMETERSET,
                  new String[]{"TALEs in FastA", "TALEs in class builder"},
                  new ParameterSet[]{new SimpleParameterSet(tales),
                        new SimpleParameterSet(builderFile)}, null, "Predictions for",
                  "Predict and intersect targets for all TALEs in a given FastA input file or for all TALEs in all classes defined by a class builder.",
                  true);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    public ToolResult run(ToolParameterSet parameters, Protocol protocol, ProgressUpdater progress,
        ...
        DataSet ds = new DataSet(new AlphabetContainer(
              new DiscreteAlphabet(true, "A", "C", "G", "T", "N", "W", "S", "M", "K", "R", "Y", "B",
                    "D", "H", "V")),
              new SparseStringExtractor(new StringReader(fr.getContent()), '>', "",
        ...
        if (selPar.getSelected() == 0) {
            ...
            try {
                TALE[] tales = ClassBuilderTool.readProteinTALEs(
                      ((FileParameter) ((ParameterSet) selPar.getValue()).getParameterAt(
                            0)).getFileContents(), protocol);
                ...
                sb = new StringBuffer();
                sb.append(((FileParameter) ((ParameterSet) selPar.getValue()).getParameterAt(
        ...
        for (int t = 0; t < sbs.length; t++) {
            ...
            DataSet tals = new DataSet(RVDAlphabetContainer.SINGLETON,
                  new SparseStringExtractor(new StringReader(sbs[t].toString()), '>', "",
                        new SimpleSequenceAnnotationParser()), "-");
        ...
        for (int s = 0; s < sbs.length; s++) {
            ...
            DataSet tals = new DataSet(model.getRVDAlphabet(),
                  new SparseStringExtractor(new StringReader(sbs[s].toString()), '>', "",
                        new SimpleSequenceAnnotationParser()), "-");
            ...
            for (int t = 0; t < tals.getNumberOfElements(); t++, totNum++) {
                ...
                talNames[t] = (String) tal.getSequenceAnnotationByType("unparsed comment line", 0)
                      .getResultForName("unparsed comment").getValue();
                ...
                if (allTales != null) {
                    if (allTales[s][t].containsAberrantRepeat()) {
                        ...
                        if (tal2.getLength() > 3) {
                            ...
                            ComparableElement<Match, Double>[] list2 = predict(model, tal2, cap, ds,
                                  offsets);

                            ComparableElement<Match, Double>[] list3 = join(tal, tal2, list, list2,
                                  cap);
                ...
                for (int i = 0; i < list.length; i++) {
                    ...
                    String id = ds.getElementAt(seqIdx)
                          .getSequenceAnnotationByType("unparsed comment line", 0)
                          .getResultForName("unparsed comment").getValue().toString().trim();

                    Sequence currTal = m.getTal() == null ? tal : m.getTal();

                    //try{
                    Sequence ts = ds.getElementAt(seqIdx)
                          .getSubSequence(m.getSeqPos(), currTal.getLength() + 1);
                    ...
                    re[i] = new ResultSet(new Result[]{
                          ...
                          new CategoricalResult("Match string", "",
                                model.getMatchString(currTal, ts) + (m.getTal() == null ? ""
                                      : "-a"))
                    });
                ...
                lires[t + off] = new ListResult("Predictions for " + talNames[t],
                      "TALgetter predictions of target sites for " + talNames[t], null, re);
            ...
            if (sbs.length == 1) {
                ...
                res[s] = new ResultSetResult("Predictions for class " + classNames[s],
                      "Predicted target sites for all TALEs in class " + classNames[s], null,
                      new ResultSet(lires));
            }
        ...
        return new ToolResult("Result of " + getToolName(),
              getToolName() + " on \"" + fr.getFilename() + "\"", null, set, parameters,
              getToolName(), new Date(System.currentTimeMillis()));
    }
        ...
        for (int i = 0; i < tales.length; i++) {
            if (tales[i].getNumberOfRepeats() > 3) {
                ...
                protocol.appendWarning(
                      "TALE " + tales[i].getId() + " ignored as it has less than 4 repeats.\n");
            }
        }
        return temp.toArray(new TALE[0]);
    }

    private static ComparableElement<Match, Double>[] join(Sequence tal, Sequence tal2,
          ComparableElement<Match, Double>[] list, ComparableElement<Match, Double>[] list2,
          int cap) {
        ...
        for (int k = list3.length - 1, i = list.length - 1, j = list2.length - 1; k >= 0; k--) {
            if (i == 0) {
                ...
                if (d1 > d2) {
                    ...
                    list3[k] = new ComparableElement<MatchFinder.Match, Double>(
                          list2[j].getElement(), d2);
                    list3[k].getElement().setTal(tal2);
                    j--;
                }
            }
    ...
    private Sequence removeAbberant(Sequence tal, TALE tale)
        ...
        for (int i = 0; i < tale.getNumberOfRepeats(); i++) {
            if (tale.getRepeat(i).getType() == TALE.Type.UNKNOWN
                  || tale.getRepeat(i).getType() == TALE.Type.NORMAL
                  || i == tale.getNumberOfRepeats() - 1) {
                tal2.append(parts[i]);
                tal2.append("-");
            }
        }
    ...
    private static ComparableElement<Match, Double>[] predict(TALgetterDiffSM model, Sequence tal,
          int cap, DataSet ds, int[][] offsets) {
        InfixMatchFinder singleFind = new InfixMatchFinder(null, Math.min(8, tal.getLength()),
              model);
        ...
        while (list.length < cap && rat > 0.1) {
            ...
            list = singleFind.getScoresAbove(tal, singleThresh * (tal.getLength() + 1), cap, true,
                  false).getSortedList();
    ...
    private ListResult intersect(DataSet ds, int[][] offsets, String[] talNames,
          Match[][] allMatches) {
        ...
        for (int i = 0; i < allMatches.length; i++) {
            for (int j = 0; j < allMatches[i].length; j++) {
                ...
                int seqIdx = m.getSeqIdx();
                int pos = m.getSeqPos() + offsets[0][seqIdx];
                String id = (String) ds.getElementAt(seqIdx)
                      .getSequenceAnnotationByType("unparsed comment line", 0)
                      .getResultForName("unparsed comment").getValue().toString().trim();
        ...
        return new ListResult("Overlapping target sites",
              "Overlapping target sites between TALEs according to TALgetter predictions", null,
              rsl.toArray(new ResultSet[0]));
    }
    ...
    public String getHelpText() {
        try {
            return FileManager.readInputStream(TALEPredictionTool.class.getClassLoader()
                        .getResourceAsStream("annotale/tools/PredictAndIntersectTargetsTool.txt"))
                  .toString();
        } catch (IOException e) {
            e.printStackTrace();
            return "";
